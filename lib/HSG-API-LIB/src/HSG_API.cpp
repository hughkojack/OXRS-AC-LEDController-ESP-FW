...
void HSG_API::loop(Client * client)
{
  _checkDisconnect();
  _checkRestart();
...
void HSG_API::get(const char * path, Router::Middleware * middleware)
{
  _api.get(path, middleware);
}

void HSG_API::post(const char * path, Router::Middleware * middleware)
{
  _api.post(path, middleware);
}

void HSG_API::onAdopt(jsonCallback callback)
{
  _apiAdopt = callback;
}

JsonVariant HSG_API::getAdopt(JsonVariant json)
{
  if (_apiAdopt)
  { 
    _apiAdopt(json);
...
void HSG_API::_initialiseRestApi(void)
{
  // /api router
  _app.use("/api", &_api);
...
void HSG_API::_checkRestart(void)
{
  if (restart) {
...
void HSG_API::_checkDisconnect(void)
{
  if (disconnect)
  {
    _eraseWifiCreds();
